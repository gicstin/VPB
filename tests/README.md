...
| `BannedApiTests` | .NET 4+ APIs that the .NET 3.5 profile VaM runs does not have, plus the project rules: no `UnityEngine.Random`, no `new System.Random(...)` — VPB uses `VpbRandom`. Also C# tuple syntax, any file that stops parsing as C# 7.3, and single-argument `StartsWith` / `EndsWith` / literal `IndexOf` that silently use the player's locale. |
...
| `LocaleSafetyTests` | Numbers across a change of thread culture: every float in `VPB.cfg` saved under en-US and read under de-DE / fr-FR / ru-RU (and back), layout preset JSON, a config written by an older build with decimal commas, outliner undo payloads restoring the exact transform and parameter value, and typed numeric input meaning the same thing in every locale. VPB loads its config in `Awake`, before VaM switches the main thread to en-US, so a German system read `0.625` as `625`. |
| `UiContrastTests` | WCAG contrast of the colour tokens that carry text: primary and on-accent labels on every selected facet fill must reach 4.5:1, secondary text on chrome surfaces, placeholders in input wells. A token nudged below that makes the selected row the hardest one to read, worse through a VR lens. |
