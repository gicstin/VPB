...
namespace VPB
{
    class Settings
    {
        ...
        private void Load(ConfigFile config)
        {
            ...
            NetClipFile = config.Bind<string>("Net", "ClipFile", "", "Clip file for record and replay. A bare name resolves inside BepInEx\\plugins\\VpbNet\\clips; a full path is used as given. Empty means: record to a timestamped name, replay the newest clip in that folder.");
